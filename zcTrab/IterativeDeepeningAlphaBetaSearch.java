...
public class IterativeDeepeningAlphaBetaSearch<S, A, P> implements AdversarialSearch<S, A> {
    ...
    protected int SEARCH_DEPTH = 7; //variavel para limitar a profundidade da busca em 7
    ...
    public A makeDecision(S state) {
        ...
        } while (!timer.timeOutOccurred() && heuristicEvaluationUsed && currDepthLimit < SEARCH_DEPTH); //limitando a profundidade de busca
        return results.get(0);
    }

    // returns an utility value
    public double maxValue(S state, P player, double alpha, double beta, int depth) {
        System.out.println("Profundidade atual max: "+depth+"\n");
        updateMetrics(depth);
    ...
    public double minValue(S state, P player, double alpha, double beta, int depth) {
        System.out.println("Profundidade atual min: "+depth+"\n");
        updateMetrics(depth);
        if (game.isTerminal(state) || depth >= currDepthLimit || timer.timeOutOccurred()) {
            return eval(state, player);
        } else {
